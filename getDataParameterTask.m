Sampling_Rate = sampleRate;    % Sampling Rate = Date flow rate, "DON'T CHANGE"
count_sample  = 0;       % Please initialize it to 0 by each trial
sample_Num    = 0;
PacketSize    = 1264;

EEGi          = 1:4:516; % calculating EEG data each 4 byte
EMGi          = 1:4:32;  % calculating EMG data each 4 byte
...
Buffer           = zeros( Buffer_FreshRate + Sampling_Rate , 8 );
BufferL= zeros( Buffer_FreshRate + Sampling_Rate , 8 );
BufferR= zeros( Buffer_FreshRate + Sampling_Rate , 8 );
%% Coefficient to change binary data to decimal data
%
% Data, which are set in uX data type, have to be changed to decimal number
%   * uX means "unsigned int X bit"
%
